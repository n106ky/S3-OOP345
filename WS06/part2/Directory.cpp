// borrowed find() from chatGPT as I don't understand the instruction
   ...
   Directory& Directory::operator+=(Resource* resource) {
      for (auto& item : m_contents) {
         if (item->name() == resource->name()) {
            throw runtime_error("Resource with the same name already exists");
         }
      }
